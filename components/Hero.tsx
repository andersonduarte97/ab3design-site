...
export default function Hero() {
  return (
    <section className="px-4 pt-4 pb-8">
      <div
        className="grain relative overflow-hidden rounded-[2.5rem]"
        style={{ height: '72vh', minHeight: '620px', backgroundColor: '#0F172A' }}
      >
        {/*
          All background elements wrapped in a single div with inline position:absolute
          so it bypasses the .grain > * { position: relative } rule in globals.css.
          Inline styles always have higher specificity than CSS class rules.
        */}
        ...
        {/* Content — normal flex flow so .grain > * doesn't break it */}
        <div className="relative flex flex-col" style={{ height: '100%', zIndex: 1 }}>

          {/* Main content — flex-1 fills available space */}
          <div className="flex-1 flex items-center px-10 md:px-20" style={{ paddingTop: '5rem' }}>
            <div className="w-full flex flex-col md:flex-row items-center gap-16">
              ...
              <div className="flex-1 min-w-0">
                {/* Headline */}
                <h1
                  className="text-5xl md:text-[5rem] font-bold text-white leading-[1.08] mb-5"
                ...
                <p
                  className="text-[#94A3B8] font-light text-lg mb-6 max-w-md leading-relaxed"
          ...
          {/* Stats bar — bottom of flex column */}
          <div
            className="px-10 md:px-20 py-5"
            style={{
              ...
              {stats.map((stat, i) => (
                <div key={stat.metric} className="flex items-center">
                  <div className="pr-8 md:pr-16">
                    <p className="text-2xl font-bold text-white leading-tight" style={{ letterSpacing: '-0.04em' }}>
                      {stat.metric}
                    </p>
                    <p className="text-xs text-white/45 font-light mt-0.5">{stat.label}</p>
                  </div>
                  {i < stats.length - 1 && (
                    <div className="w-px h-10 bg-white/10 mr-8 md:mr-16 shrink-0" />
                  )}
                </div>
              ))}
            </div>
